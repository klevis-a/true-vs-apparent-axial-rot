...
from true_vs_apparent.common.plot_utils import style_axes
from true_vs_apparent.common.analysis_utils import extract_sub_rot
import logging
...
def summary_plotter(shoulder_trajs, traj_def, x_ind_def, y_ind_def, x_cmn_def, y_cmn_def, decomp_method, sub_rot, ax,
                    ind_plotter_fnc, avg_color, quat_avg_color, error_bars='se', **error_bar_kwargs):
    ...
    if error_bars.lower() == 'se':
        agg_lines = ax.errorbar(x_cmn, np.rad2deg(traj_mean), yerr=np.rad2deg(traj_se), capsize=2, color=avg_color,
                                zorder=4, lw=2, **error_bar_kwargs)
    elif error_bars.lower() == 'std':
        agg_lines = ax.errorbar(x_cmn, np.rad2deg(traj_mean), yerr=np.rad2deg(traj_std), capsize=2, color=avg_color,
                                zorder=4, lw=2, **error_bar_kwargs)
    else:
