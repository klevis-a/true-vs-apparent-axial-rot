"""Compare yx'y'' and zx'y'' GH, HT, ST (HT-GH) axial rotation between planes of elevation.
...
if __name__ == '__main__':
    ...
    config_dir = Path(mod_arg_parser("Compare yx'y'' and zx'y'' GH, HT, ST (HT-GH) axial rotation between planes of "
    ...
    def get_trajs_gh(df):
        ...
        all_traj_phadke = np.stack(df['traj_interp'].apply(
            extract_sub_rot, args=['gh', 'common_fine_up', 'euler.gh_phadke', 2]), axis=0)
        all_traj_true = np.stack(df['traj_interp'].apply(
            extract_sub_rot, args=['gh', 'common_fine_up', 'true_axial_rot', None]), axis=0)

        return all_traj_isb, all_traj_phadke, all_traj_true

    def get_trajs_ht(df):
        all_traj_isb = np.stack(df['traj_interp'].apply(
            extract_sub_rot, args=['ht', 'common_fine_up', 'euler.gh_isb', 2]), axis=0)
        all_traj_phadke = np.stack(df['traj_interp'].apply(
            extract_sub_rot, args=['ht', 'common_fine_up', 'euler.gh_phadke', 2]), axis=0)
        all_traj_true = np.stack(df['traj_interp'].apply(
            extract_sub_rot, args=['ht', 'common_fine_up', 'true_axial_rot', None]), axis=0)

        return all_traj_isb, all_traj_phadke, all_traj_true

    def get_trajs_st(df):
        all_traj_isb_ht, all_traj_phadke_ht, all_traj_true_ht = get_trajs_ht(df)
        all_traj_isb_gh, all_traj_phadke_gh, all_traj_true_gh = get_trajs_gh(df)

        return (all_traj_isb_ht - all_traj_isb_gh, all_traj_phadke_ht - all_traj_isb_gh,
                all_traj_true_ht - all_traj_true_gh)

    def get_trajs_gh_start(df):
        all_traj_isb = np.stack(df['traj_interp'].apply(
    ...
    def get_trajs_ht_start(df):
        all_traj_isb = np.stack(df['traj_interp'].apply(
            extract_sub_rot_norm, args=['ht', 'common_fine_up', 'euler.gh_isb', 2, 'up']), axis=0)
        all_traj_phadke = np.stack(df['traj_interp'].apply(
            extract_sub_rot_norm, args=['ht', 'common_fine_up', 'euler.gh_phadke', 2, 'up']), axis=0)
        all_traj_true = np.stack(df['traj_interp'].apply(
            extract_sub_rot_norm, args=['ht', 'common_fine_up', 'true_axial_rot', None, 'up']), axis=0)

        return all_traj_isb, all_traj_phadke, all_traj_true

    def get_trajs_st_start(df):
        all_traj_isb_ht, all_traj_phadke_ht, all_traj_true_ht = get_trajs_ht_start(df)
        all_traj_isb_gh, all_traj_phadke_gh, all_traj_true_gh = get_trajs_gh_start(df)
    ...
    plot_dir = {'GH': (get_trajs_gh, get_trajs_gh_start),
    ...
    for traj, traj_fnc in plot_dir.items():
        ...
        all_traj_isb_rm, all_traj_phadke_rm, all_traj_true_rm = traj_fnc[0](db_elev_equal)
        ...
        for idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
            all_traj_isb_act, all_traj_phadke_act, all_traj_true_act = traj_fnc[0](activity_df)
        ...
        fig_plane.suptitle(traj + ' Axial Rotation Comparison by Plane', x=0.5, y=0.99, fontweight='bold')
        plt.subplots_adjust(top=0.93)
        axs_plane[0, 0].legend(mean_lns, activities, loc='upper left', bbox_to_anchor=(0, 1.05), ncol=3,
        ...
        all_traj_isb_start, all_traj_phadke_start, all_traj_true_start = traj_fnc[1](db_elev)
        group = (db_elev['Activity'].map({'CA': 1, 'SA': 2, 'FE': 3})).to_numpy(dtype=np.int)
        ...
        spm_one_way_phadke_start = spm1d.stats.anova1(all_traj_phadke_start,
                                                      group, equal_var=False).inference(alpha=0.05)

        all_traj_isb_rm_start, all_traj_phadke_rm_start, all_traj_true_rm_start = traj_fnc[1](db_elev_equal)
        group_rm = (db_elev_equal['Activity'].map({'CA': 1, 'SA': 2, 'FE': 3})).to_numpy(dtype=np.int)
        subj_rm = (db_elev_equal['Subject_Short'].map(subj_name_to_number)).to_numpy()
        spm_one_way_rm_true_start = spm1d.stats.anova1rm(all_traj_true_rm_start,
        ...
        one_way_rm_ln_true_start = spm_plot(axs_plane_start[0, 1], x, spm_one_way_rm_true_start,
                                            shaded_spm_rm, line_spm_rm)
        one_way_rm_ln_isb_start = spm_plot(axs_plane_start[1, 1], x, spm_one_way_rm_isb_start,
                                           shaded_spm_rm, line_spm_rm)
        ...
        activities_start = []
        for idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
            all_traj_isb_act, all_traj_phadke_act, all_traj_true_act = traj_fnc[1](activity_df)
        ...
        # figure title and legend
        plt.tight_layout(pad=0.25, h_pad=1.5, w_pad=0.5)
        fig_plane_start.suptitle(traj + ' Axial Rotation Comparison by Plane Norm Start', x=0.5, y=0.99,
                                 fontweight='bold')
        plt.subplots_adjust(top=0.93)
        ...
        axs_plane_start[0, 1].legend([one_way_ln_true[0], one_way_rm_ln_true[0]], ['SPM {t}', 'SPM {t} RM'],
                                     loc='lower left', bbox_to_anchor=(0, 0.1), ncol=2, handlelength=1.5,
        ...
        spm_one_way_rm_phadke_norm_start = spm1d.stats.normality.sw.anova1rm(all_traj_phadke_rm_start,
                                                                             group_rm, subj_rm)
        ...
        norm_rm_true_ln_start = ax_norm_start.plot(x, spm_one_way_rm_true_norm_start[1],
                                                   color=color_map.colors[0], ls='--')
        norm_rm_isb_ln_start = ax_norm_start.plot(x, spm_one_way_rm_isb_norm_start[1],
                                                  color=color_map.colors[1], ls='--')
