"""Compare GH yx'y'' and zx'y'' GH axial rotation against true axial rotation normalized by their values at 25 deg of
HT elevation.
...
if __name__ == '__main__':
    ...
    config_dir = Path(mod_arg_parser("Compare GH yx'y'' and zx'y'' GH axial rotation against true axial rotation "
    ...
    # plot
    spm_y = np.array([[38, 38], [28, 28], [23, 23]])
    max_pos = 140
    leg_left_mean = []
    leg_right_mean = []
    for idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
        all_traj_isb = np.stack(activity_df['traj_interp'].apply(
            extract_sub_rot_norm, args=['gh', 'common_fine_up', 'euler.gh_isb', 2, 'common_fine_up']), axis=0)
        all_traj_isb_poe = np.stack(activity_df['traj_interp'].apply(
            extract_sub_rot_norm, args=['gh', 'common_fine_up', 'euler.gh_isb', 0, 'common_fine_up']), axis=0)
        all_traj_phadke = np.stack(activity_df['traj_interp'].apply(
            extract_sub_rot_norm, args=['gh', 'common_fine_up', 'euler.gh_phadke', 2, 'common_fine_up']), axis=0)
        all_traj_true = np.stack(activity_df['traj_interp'].apply(
            extract_sub_rot_norm, args=['gh', 'common_fine_up', 'true_axial_rot', None, 'common_fine_up']), axis=0)
        ...
        all_traj_isb_poe_max = np.stack(activity_df['traj_interp'].apply(
            sub_rot_at_max_elev, args=['gh', 'euler.gh_isb', 0, 'common_fine_up']), axis=0)
        ...
        # spm
        # ignore the first data point because it will always be zero
    ...
    axs_diff[0, 0].arrow(35, 28, 0, -17, length_includes_head=True, head_width=2, head_length=2)
    axs_diff[0, 0].text(23, 28, 'External\nRotation', rotation=90, va='top', ha='left', fontsize=10)
