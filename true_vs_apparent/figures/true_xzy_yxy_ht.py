"""Compare HT yx'y'' and zx'y'' GH axial rotation against true axial rotation.
...
if __name__ == '__main__':
    ...
    # set axes limits
    ax_limits_left = [(-80, 10), (-100, 15), (-155, 43)]
    ax_limits_right = [(-65, 60), (-58, 50), (-90, 43)]
    for i in range(3):
    ...
    # plot
    spm_y = np.array([[5, 55], [10, 45], [38, 38]])
    max_pos = 140
    leg_left_mean = []
    leg_right_mean = []
    for idx, (activity, activity_df) in enumerate(db_elev.groupby('Activity', observed=True)):
        ...
        all_traj_isb_poe = np.stack(activity_df['traj_interp'].apply(
            extract_sub_rot_norm, args=['ht', 'common_fine_up', 'euler.gh_isb', 0, 'up']), axis=0)
        ...
        all_traj_isb_max = np.stack(activity_df['traj_interp'].apply(
            sub_rot_at_max_elev, args=['ht', 'euler.gh_isb', 2, 'up']), axis=0)
        all_traj_isb_poe_max = np.stack(activity_df['traj_interp'].apply(
            sub_rot_at_max_elev, args=['ht', 'euler.gh_isb', 0, 'up']), axis=0)
        all_traj_phadke_max = np.stack(activity_df['traj_interp'].apply(
            sub_rot_at_max_elev, args=['ht', 'euler.gh_phadke', 2, 'up']), axis=0)
        all_traj_true_max = np.stack(activity_df['traj_interp'].apply(
            sub_rot_at_max_elev, args=['ht', 'true_axial_rot', None, 'up']), axis=0)
    ...
    for i in range(3):
        labels = [item.get_text() for item in axs_diff[i, 1].get_xticklabels()]
        empty_string_labels = [''] * len(labels)
        axs_diff[i, 1].set_yticklabels(empty_string_labels)
